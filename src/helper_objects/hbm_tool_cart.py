"""HBM 146cm tool cart — CadQuery model.
...
Confirmed dimensions (from product spec):
  Outer (incl. wielen + handvat):  1610 x 460 x 920 mm  (L x B x H)
  Body only (excl. accessoires):   1465 x 460 x 770 mm
...
CART_BODY_HEIGHT = CART_BODY_HEIGHT_NO_WHEEL
CART_TOP_OVERHANG = (CART_TOTAL_LENGTH - CART_BODY_LENGTH) // 2
...
def _caster(wheel_diam: float, wheel_w: float, total_h: float):
    """Swivel caster: wheel + mounting stem above it."""
    r = wheel_diam / 2
    wheel = cq.Workplane("XZ").circle(r).extrude(wheel_w / 2, both=True).translate((0, 0, r))
    fork_h = total_h - wheel_diam
    fork = (
        cq.Workplane("XY").box(20, wheel_w + 10, fork_h).translate((0, 0, wheel_diam + fork_h / 2))
    )
...
def make_hbm_tool_cart(
    ...
    assy.add(
        body, name="body", color=Color(0.08, 0.08, 0.08, 1.0), loc=Location(Vector(0, 0, body_z))
    )

    # ── Drawer layout ──────────────────────────────────────────────────────────
    col_specs = [(-450, 330), (0, 570), (450, 330)]
    row_heights = [45, 150, 45, 150, 45, 200]
    ...
    assy.add(
        top, name="wood_top", color=Color(0.72, 0.52, 0.30, 1.0), loc=Location(Vector(0, 0, wood_z))
    )
    ...
    arm_z = wheel_height + body_h * 0.78
    arm_length = 131
    span_y = 160
    r = 14
    ...
    assy.add(
        handle_shape,
        name="handle",
        color=Color(0.78, 0.78, 0.78, 1.0),
